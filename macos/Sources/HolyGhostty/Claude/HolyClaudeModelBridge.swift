...
enum HolyClaudeModelBridge {
    ...
    # Session and rate-limit fields are optional: `rate_limits` is present only
    # for claude.ai subscribers after the first API response. Each window is
    # read as two numbers and used only if both parse. session_id is a UUID
    # (36 characters); cwd is bounded by PATH_MAX (1024 on macOS).
    if command -v jq >/dev/null 2>&1; then
      fields=$(printf '%s' "$input" | jq -r '[
        (.hook_event_name // ""),
        ((.model.display_name // .model.id // "") | gsub("[^A-Za-z0-9 ._+/@:-]"; "") | .[0:48]),
        ((.effort.level // "") | gsub("[^A-Za-z0-9_ -]"; "") | .[0:12]),
        ((.session_id // "") | gsub("[^A-Za-z0-9_-]"; "") | .[0:36]),
        ((.rate_limits.five_hour.used_percentage // "") | tostring),
        ((.rate_limits.five_hour.resets_at // "") | tostring),
        ((.rate_limits.seven_day.used_percentage // "") | tostring),
        ((.rate_limits.seven_day.resets_at // "") | tostring),
        ((.workspace.current_dir // .cwd // "") | gsub("[^A-Za-z0-9 ._+/@:~-]"; "") | .[0:1024])
      ] | @tsv' 2>/dev/null)
      tab=$(printf '\t')
      event=${fields%%"$tab"*}; remainder=${fields#*"$tab"}
      model=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      effort=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      session_id=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      five_pct=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      five_reset=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      seven_pct=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      seven_reset=${remainder%%"$tab"*}; remainder=${remainder#*"$tab"}
      cwd=$remainder
      [ "$fields" != "$event" ] || { model=""; effort=""; session_id=""; five_pct=""; five_reset=""; seven_pct=""; seven_reset=""; cwd=""; }
    else
      ...
      session_id=$(printf '%s' "$input" | /usr/bin/plutil -extract session_id raw -o - - 2>/dev/null | LC_ALL=C tr -cd '[:alnum:]_-' | cut -c1-128)
      five_pct=$(printf '%s' "$input" | /usr/bin/plutil -extract rate_limits.five_hour.used_percentage raw -o - - 2>/dev/null)
      five_reset=$(printf '%s' "$input" | /usr/bin/plutil -extract rate_limits.five_hour.resets_at raw -o - - 2>/dev/null)
      seven_pct=$(printf '%s' "$input" | /usr/bin/plutil -extract rate_limits.seven_day.used_percentage raw -o - - 2>/dev/null)
      seven_reset=$(printf '%s' "$input" | /usr/bin/plutil -extract rate_limits.seven_day.resets_at raw -o - - 2>/dev/null)
      cwd=$(printf '%s' "$input" | /usr/bin/plutil -extract workspace.current_dir raw -o - - 2>/dev/null | LC_ALL=C tr -cd '[:alnum:] ._+/@:~-' | cut -c1-1024)
    fi
    # Numbers only; anything else means the window was absent.
    five_pct=$(printf '%s' "$five_pct" | LC_ALL=C tr -cd '0-9.')
    five_reset=$(printf '%s' "$five_reset" | LC_ALL=C tr -cd '0-9')
    seven_pct=$(printf '%s' "$seven_pct" | LC_ALL=C tr -cd '0-9.')
    seven_reset=$(printf '%s' "$seven_reset" | LC_ALL=C tr -cd '0-9')
    ...
    if [ "$event" = "SessionEnd" ]; then
      ...
      usage_sessions="${HOLY_USAGE_DIR:-$HOME/Library/Application Support/Holy Ghostty/usage}/sessions"
      [ -z "$session_id" ] || rm -f "$usage_sessions/$session_id.json" 2>/dev/null || true
      exit 0
    fi

    # Publish this session's own windows for Holy's usage guard. They come
    # from this session's API responses, so they stay right for the account
    # the session is actually running under.
    usage_sessions="${HOLY_USAGE_DIR:-$HOME/Library/Application Support/Holy Ghostty/usage}/sessions"
    usage_suffix=""
    if [ -n "$session_id" ] && [ -d "$usage_sessions" ] && { [ -n "$five_pct" ] || [ -n "$seven_pct" ]; }; then
      now=$(date +%s)
      json="{\"t\":$now,\"session_id\":\"$session_id\",\"pane\":\"${TMUX_PANE:-}\",\"cwd\":\"$cwd\""
      if [ -n "$five_pct" ]; then
        json="$json,\"five_hour\":{\"percent\":$five_pct,\"resets_at\":${five_reset:-null}}"
        usage_suffix="$usage_suffix · 5h ${five_pct%.*}%"
      fi
      if [ -n "$seven_pct" ]; then
        json="$json,\"seven_day\":{\"percent\":$seven_pct,\"resets_at\":${seven_reset:-null}}"
        usage_suffix="$usage_suffix · wk ${seven_pct%.*}%"
      fi
      json="$json}"
      tmp="$usage_sessions/.$session_id.$$.tmp"
      if printf '%s\n' "$json" > "$tmp" 2>/dev/null; then
        mv -f "$tmp" "$usage_sessions/$session_id.json" 2>/dev/null || rm -f "$tmp"
      fi
    fi
