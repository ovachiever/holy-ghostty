...
struct HolyAgentStateBridgeTests {
    ...
    @Test func codexNotifyConfigurationPreservesBytesAndFailsClosedOnForeignValues() throws {
        ...
        // A foreign notifier that chains Holy's adapter (Codex Computer Use
        // style --previous-notify) is a delegation: both merge and remove
        // leave the whole configuration untouched, even with a stale Holy
        // marker elsewhere in the file, and never block the install.
        let delegated = "# Generated by Holy Ghostty. Owner: com.holyghostty.codex-notify-config.v1\n\n"
            + "approval_policy = \"never\"\n\n"
            + #"notify = ["/x/SkyComputerUseClient", "turn-ended", "--previous-notify", "[\"\\/x\\/holy-codex-turn-complete.py\"]"]"#
            + "\n"
        #expect(try HolyAgentStateBridge.mergingCodexConfiguration(
            delegated,
            adapterURL: adapterURL
        ) == delegated)
        #expect(try HolyAgentStateBridge.removingCodexConfiguration(
            delegated,
            adapterURL: adapterURL
        ) == delegated)
        #expect(throws: HolyCodexNotifyConfigurationError.foreignNotify) {
