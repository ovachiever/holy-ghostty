...
struct HolyAgentStateBridgeTests {
    ...
    func committedUserPromptTraversesRealTmuxMonitorAndAdvancesHumanRecency() async throws {
        let tmuxURL = try #require(holyAgentStateRealTmuxURL)
        let socketName = "hup-\(UUID().uuidString.prefix(8))"
        let sessionName = "prompt-roundtrip"
        defer {
            _ = try? runCommand(
                tmuxURL,
        ...
        let serverEnvironment = cleanTmuxEnvironment()
        #expect(try runCommand(
            tmuxURL,
            arguments: [
                "-L", socketName,
                "-f", "/dev/null",
        ...
        let paneResult = try runCommand(
            tmuxURL,
            arguments: ["-L", socketName, "display-message", "-p", "-t", sessionName, "#{pane_id}"],
            environment: serverEnvironment
        )
        let paneID = paneResult.output.trimmingCharacters(in: .whitespacesAndNewlines)
        #expect(paneResult.status == 0)

        let committedEnvelope = try HolyAgentStateEnvelope(
            source: HolyAgentStateSource.claude,
            lifecycle: .working,
            occurredAt: Date(),
            eventToken: "real-tmux-prompt",
        ...
        #expect(try runCommand(
            ...
            arguments: [
                "-L", socketName,
                "set-option", "-pq", "-t", paneID,
                HolyAgentStateTransport.tmuxOption, committedEnvelope.wireValue,
            ],
            environment: serverEnvironment
        ).status == 0)
        ...
        let observations = try await HolyTmuxAgentStateMonitor().read(endpoint: endpoint)
        let observation = try #require(observations.values.first {
            $0.key.sessionName == sessionName
        })
        let envelope = try #require(observation.envelope)
        #expect(envelope.reasonCode == "user-prompt")

        var metadata = HolySessionAttentionMetadata(sessionID: UUID())
        _ = metadata.migrateToCurrentSeenTracking(at: observation.observedAt)
        let didRecord = metadata.record(envelope: envelope, observedAt: observation.observedAt)
        #expect(didRecord)
        #expect(metadata.lastHumanUsedAt == envelope.occurredAt)
        #expect(metadata.lastAgentActiveAt == envelope.occurredAt)
    }
