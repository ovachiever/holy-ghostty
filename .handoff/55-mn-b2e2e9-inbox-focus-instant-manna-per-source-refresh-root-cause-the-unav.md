...
scope: '[INBOX] Focus-instant manna, per-source refresh, root-cause the unavailable GitHub sweep'
inputs: []
binding: sha256:da5701c9ba7ee9ae4098b4b74ac949befb42a3f9a66422c025084d61de4638a8
---

# Handoff: [INBOX] Focus-instant manna, per-source refresh, root-cause the unavailable GitHub sweep
...
```bash
agent-do manna claim mn-b2e2e9
```

## Scope

[INBOX] Focus-instant manna, per-source refresh, root-cause the unavailable GitHub sweep
...
3. Seal changes with `agent-do manna handoff seal mn-b2e2e9`.
4. Commit with `Manna: mn-b2e2e9` and run `agent-do manna done mn-b2e2e9` only after the work is verified.
