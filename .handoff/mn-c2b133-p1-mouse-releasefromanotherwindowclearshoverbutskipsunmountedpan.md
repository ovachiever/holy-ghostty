...
implementation, not green on this correction.

- Correction commit in primary: `61da5f00257a5f6889f5dd32b1a63e3072339334`
  (`fix(terminal): check pane bounds for window overlap`, exact trailer
  `Manna: mn-c2b133`). Isolated commit: `567cc679e` in the existing
  `/Users/erik/Custom-Coding/holy-ghostty-codex-mn-c2b133` worktree.
- The existing Manna claim and the prior sealed binding
  `sha256:935432302f7987c373fe9f89c2bffcf9af49145f5b96318d040fbe2d1f42e73a`
  were verified before editing. Coord ownership was renewed for this correction.
- Coordinated failure evidence: `.dev/acceptance-c2b133-f4ab17/RECEIPT.md` and
  `combined.xcresult`, executed at `a5b24f978`. Both mouse-release regressions
  failed on both parallel hosts. XCResult's summary counts 65 passed, 2 failed,
  0 skipped (67 unique tests across the combined mouse/Board selection); the
  verifier's raw console counts are 140 passed and 4 failed execution events.
  The failed expectations are `clipped.modifiers.isEmpty` at line 105 and
  `outside.modifiers.isEmpty` at line 140.
- The discriminating AppKit probe in that receipt gives
  `clipped.visibleRect = (-1000, 0, 600, 432)`. Under non-clipping ancestors,
  converting that rectangle into content coordinates returns the window area,
  so the initial guard could not exclude an off-window pane. The initial
  geometry diagnosis and guard below are superseded by this executed evidence.
- The production path now uses exactly:
  ...
  The existing `!surface.visibleRect.isEmpty` guard remains for clipping
  hierarchies. Hidden, foreign-window, detached, and first-responder checks
  remain in place. No test assertion was weakened or changed.
- The six probe fixtures from the coordinator map to the corrected path:
  visible and partially visible panes forward; off-window, edge-only, foreign,
  and detached panes skip. The partially visible frame is `(550, 50, 100, 100)`;
  the edge-only frame begins at content x=600. These are the coordinator's
  measured probe outcomes and the unchanged suite expectations, not a new
  executed test result from this lane.

Validation from the preserved isolated worktree:
...
/usr/bin/xcodebuild -quiet -project macos/Ghostty.xcodeproj -scheme Ghostty \
  ...
  -resultBundlePath .dev/mn-c2b133/revision-2/build-for-testing.xcresult \
...
All four commands exited 0. Strict lint: zero violations in the controller and
mouse fixtures. Xcode: succeeded, zero errors, 37 warnings elsewhere and zero
warnings in those two files. Both focused suites compiled, including all six
expanded mouse cases. **Tests executed by this correction lane: 0** under the
continuing no-launch boundary. No app launches, installs, screenshots, live
session spawns, pushes, or PRs occurred.

Compiled controller SHA-256:
`0f84ee7d30da7bb45bea03699b0ad4072826560178ecfc1cae4da2e6d08f5d6f`.
Unchanged mouse test SHA-256:
`e583d0eda82d874c012820406d2f7e340b8cf2fc1e02eaf6ca06afb56848581b`.
Both match the committed primary files. New receipts are in
`.dev/mn-c2b133/revision-2/`: `verification.json`, `core-verify.log`,
`swiftlint.log`, `build-for-testing.log`, `build-for-testing.xcresult`,
`build-results.json`, and `previous-executed-summary.json`.

Hand execution back through Coord need `mn-c2b133-executed-acceptance` and the
drop to verifier `session-17a022e17710`. From the preserved worktree, repeat the
Xcode command above with action `test-without-building` and a fresh result path
`.dev/mn-c2b133/revision-2/executed-acceptance.xcresult`. Keep both suite filters.
That command has not run here. Append the executed counts and result bundle,
reseal, and mark done only on verified acceptance. The existing URL visual pass
stays coordinated at wave close.

Retracted the initial incorrect geometry solution in ZPC lesson `les-034485`
with the executed-red receipt; recorded the correction as `les-64d59b`.
Lessons logged: 1 (new this pass; 7 total) | Decisions logged: 0 (new).

## Historical initial implementation receipt: 2026-09-10

The following records the initial compiled-only change. Revision 2 above
supersedes its geometry claim and implementation; the original evidence is
